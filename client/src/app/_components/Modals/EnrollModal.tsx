...
const EnrollModal = ({ targetProject }: { targetProject: Project }) => {
  ...
  async function submitEnroll() {
    axios
      ...
      .then(async (res) => {
        ...
        toggleNotification(
          "Enroll Success",
          `Successfully enrolled into project ${targetProject.name}!`,
          "success",
        );
  ...
  const openModal = async (e: SyntheticEvent, enrolledCode: number) => {
    e.stopPropagation();
    let currentEnrolled = await axios.get(
      `http://localhost:3500/projects/${enrolledCode}`,
    );
    modals.openConfirmModal({
      ...
      children: (
        <>
          <Text size="sm">You are already enrolled into project:</Text>
          <Text size="sm" c="blue">
            {currentEnrolled.data.code} - {currentEnrolled.data.name}
            <span className="text-green-600">
              {currentEnrolled.data.status === "APPROVED"
                ? ""
