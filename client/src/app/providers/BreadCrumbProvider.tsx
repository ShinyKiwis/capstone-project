...
const BreadCrumbProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  const buildBreadCrumbs = (program?: Program, version?: Version, so?: SO) => {
    ...
    if (path.includes("/program")) {
      breadCrumbsArray = buildProgramBreadCrumbs(ids, program, version, so);
    }
    console.log("BREAD:", breadCrumbsArray);
    setBreadCrumbs(breadCrumbsArray);
  };

  const buildProgramBreadCrumbs = (ids: string[], program?: Program, version?: Version, so?: SO) => {
    const path = window.location.pathname;
