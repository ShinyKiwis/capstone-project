  ...
  useEffect(() => {
    ...
    if (SOnames.length < 1) {
      ...
      selectedSOs.forEach((SO) => {
        SO.performanceIndicators.sort(compare);
        if (!SOnames.includes(SO.name)) {
          namesList.push(SO.name);
        }
      });

      // console.log("Extracted names:", namesList);
      setSOnames(namesList);
      setexpandedSOs(namesList);
    }
  }, []);
      ...
      <div className="mt-4 h-full overflow-auto pb-4">
        ...
        >
          {selectedSOs!.map((studentOutcome, SOindex) => {
            ...
            return (
              <Accordion.Item
                value={studentOutcome.name}
                key={studentOutcome.id}
              >
                <Accordion.Control>
                          ...
                          return (
                            <div className="flex items-center gap-1">
                              <NumberInput
                                ...
                                {...form2.getInputProps(
                                  `SOs.${SOindex}.performanceIndicators.${PIindex}.passingGoal`,
                                )}
                              />
                              <Text>%</Text>
                            </div>
